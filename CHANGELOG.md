...
### Fixed
- **Atlassian API token auth broken** — tokens with `ATATT` prefix (Atlassian HTTP Access Tokens) were sent as Bearer auth, which Bitbucket's REST API rejects; they now correctly use Basic auth (username + token as password)
- **Bearer auth header silently overwritten** — duplicate `headers` keys in the Axios config object meant the `Authorization: Bearer` header was always replaced by the outer `headers: { Accept, Content-Type }` block; auth headers are now merged correctly
