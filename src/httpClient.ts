...
function buildClient(): AxiosInstance {
  // Atlassian API tokens (ATATT prefix) require Basic auth; other tokens use Bearer.
  const hasApiToken = !!config.bitbucketApiToken;
  const isAtlassianToken = hasApiToken && config.bitbucketApiToken!.startsWith('ATATT');
  const useBasicAuth = !hasApiToken || isAtlassianToken;

  const authConfig = useBasicAuth
    ? {
        auth: {
          username: config.bitbucketUsername!,
          password: (config.bitbucketApiToken || config.bitbucketAppPassword)!,
        },
      }
    : {};
  ...
  const client = axios.create({
    ...
    headers: {
      ...
      ...(!useBasicAuth && config.bitbucketApiToken
        ? { Authorization: `Bearer ${config.bitbucketApiToken}` }
        : {}),
    },
    timeout: 30_000, // 30s per request
  });

  log.info({ authMethod: useBasicAuth ? (isAtlassianToken ? 'atlassian-token-basic' : 'app-password') : 'bearer-token' }, 'HTTP client initialised');
